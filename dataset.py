    ...
    class CustomTensorDataset(Dataset):
        def __init__(self, x_train, y_train, transform=None):
            ...
            if x_train.shape[1:] == (28,28):
                ...
                #self.transform2 = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=[x / 255.0 for x in [125.3, 123.0, 113.9]],std=[x / 255.0 for x in [63.0, 62.1, 66.7]])])
...
def get_bigdata_torch(dataset_name,random_seed):
    ...
    #         transforms.ToTensor(),
    #         transforms.Normalize((0.485, 0.456, 0.406),(0.229, 0.224, 0.225))((0.6959, 0.6537, 0.6371),(0.3113, 0.3192, 0.3214))
    #     ]) 
    ...
    meta_ds = torch_dataset(meta_imgs,meta_labels,transform_train,len(class_names))
    return train_ds, val_ds, test_ds, meta_ds, class_names
    ...
    if dataset_name in DATASETS_SMALL:
        ...
        if framework == 'tensorflow':
            ...
            if not (x_meta is None):
                ...
                meta_dataset = CustomTensorDataset(x_meta, y_meta, transform=transorm_func)
